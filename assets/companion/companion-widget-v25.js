(() => {
  ...
  let motionRequested = false, failedAction = null;
  ...
  function armIdle() {
    ...
    if (!state.open || !state.ready || state.mode !== 'idle' || state.menu || state.closing || yawnUsed || failedAction || !allowMotion() || document.hidden) return;
    idleTimer = setTimeout(() => {
      if (!state.open || state.mode !== 'idle' || state.menu || document.hidden || yawnUsed) return;
      yawnUsed = true;
      run('yawn');
    }, Math.max(0, 10000 - (Date.now() - lastActivity)));
  }
  function activity() {
  ...
  function returnToIdle() {
    ...
    if (!allowMotion()) { transition++; player.still(); state.mode = 'idle'; state.busy = false; render(); armIdle(); }
    else run('idle');
  }
  ...
  function playRequestedAction(name) {
    state.menu = false;
    if (name === 'yawn') yawnUsed = true;
    run(name, { sound: true });
  }
  async function open(greeting = true, action = null) {
    if (greeting || action) { motionRequested = true; player.unlock(); }
    activity();
  ...
  $('.retry').addEventListener('click', () => {
    if (failedAction && state.ready) {
      ...
      motionRequested = true; player.unlock(); activity(); run(action.name, { sound: action.sound });
    } else { state.open = false; open(); }
  });
  $('.dance').addEventListener('click', () => { motionRequested = true; activity(); player.unlock(); if (state.mode === 'dance') returnToIdle(); else run('dance'); });
  $('.voice').addEventListener('click', () => { player.toggleMuted(); activity(); });
  actor.addEventListener('click', event => {
    if (suppressClick && event.detail !== 0) { suppressClick = false; return; }
    motionRequested = true; activity();
    if (state.mode === 'idle' && !state.busy && !player.current && !failedAction) returnToIdle();
    state.menu = !state.menu; render(); armIdle();
  });
