  ...
  class CompanionPlayer {
    ...
    async fetchFile(name, kind) {
      ...
      try {
        ...
        if (!response.ok) throw Error('無法載入角色素材');
    ...
    load() {
      ...
      this.loading = this.fetchFile('clips-v23.json', 'json').then(async clips => {
        this.clips = clips;
    ...
    async play(name, { sound = name !== 'idle', onEnd = () => {} } = {}) {
      ...
      try {
        [prepared] = await Promise.all([
          this.prepare(name, sound && !this.muted && !!this.context),
          this.resuming
        ]);
      }
      catch (error) {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.image.onload = () => {
          ...
          this.current = { name, sound, started: performance.now(), duration: prepared.clip.duration, epoch };
          this.element.dataset.playing = 'true';
          this.startSound(prepared.buffer, epoch);
    ...
    toggleMuted() {
      ...
      if (this.muted) this.stopSound();
        ...
        this.unlock();
        const current = this.current;
        if (current?.sound) Promise.all([this.prepare(current.name, true), this.resuming]).then(([{ buffer }]) => {
          if (this.current === current) this.startSound(buffer, current.epoch);
        }).catch(() => {});
      }
      this.notify();
    }
