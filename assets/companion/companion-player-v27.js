(() => {
  ...
  class CompanionPlayer {
    ...
    async play(name, { sound = name !== 'idle', onEnd = () => {}, onCycle = () => {} } = {}) {
      this.cancel();
        ...
        this.image.onload = () => {
          ...
          const completeCycle = () => {
            ...
            if (prepared.clip.loop === 0) {
              // Report completed idle cycles without restarting its one-shot voice.
              this.current.sound = false;
              this.notify();
              onCycle();
              // A callback may have replaced this clip or closed the companion.
              if (epoch === this.epoch && this.current) this.timer = setTimeout(completeCycle, prepared.clip.duration);
              return;
            }
            this.current = null;
            this.element.dataset.playing = 'false';
          ...
          this.timer = setTimeout(completeCycle, prepared.clip.duration);
          this.notify();
          resolve(true);
        };
        this.image.onerror = () => {
